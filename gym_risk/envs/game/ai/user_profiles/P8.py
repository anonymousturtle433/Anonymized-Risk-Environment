...
class P8(AI):
    ...
    def initial_placement(self, empty, remaining):
        ...
        '''
        # empty_names = [territory.name for territory in empty]
        if self.map_name != 'RANDOM':
            if self.locations:
                t = self.locations[0]
    ...
    def reinforce(self, available):
        not_border = [t for t in self.player.territories if not t.border]
        LOG.info(not_border)
        if len(not_border) == 0:
            not_border = list(self.player.territories)
        result = collections.defaultdict(int)
        for i in range(available):
            t = random.choice(not_border)
            result[t] += 1
        return result
