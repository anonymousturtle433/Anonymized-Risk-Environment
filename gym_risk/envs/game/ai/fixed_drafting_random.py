...
class FixedDraftingRandomAI(AI):
    """
     FixedDraftingRandomAI: Plays a semi random game, selecting predetermined troop positions, then randomly choosing and 
     reinforcing territories, and attacking wherever it can without any considerations of wisdom during the main phase of the game.
    """
    def __init__(self, player, game, world, **kwargs):
        ...
        #adjusts total location amount from MAP_Deployments to be reinforcement amount
            self.reinforcment_amounts = {location:amount-1 for (location, amount) in self.reinforcment_amounts.items() if amount-1>0}
        self.player = player
    ...
    def initial_placement(self, empty, remaining):
        ...
        Alternatively, the map can be randomly generarted, if the 'map_name' parameter is random.
